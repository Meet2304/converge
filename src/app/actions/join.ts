...
const NICKS = [
  "Signal Fox", "Quiet Comet", "Neon Relay", "Copper Finch", "Amber Grid",
  "Pixel Heron", "Volt Moth", "Cache Wren", "Prism Hare", "Delta Kite",
]
...
export async function enterEventCode(formData: FormData) {
  ...
  const { data } = await db.from("events").select("id, share_code").eq("share_code", code).maybeSingle()
...
export async function setLooking(formData: FormData) {
  ...
  const mine =
    (ctx.user && part.user_id === ctx.user.id) ||
    part.anon_session_id === ctx.anonSessionId
