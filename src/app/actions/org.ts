...
export async function createOrganization(formData: FormData) {
  ...
  const { data: clash } = await db.from("organizations").select("id").eq("slug", slug).maybeSingle()
...
export async function createEvent(formData: FormData) {
  ...
  for (let i = 0; i < 5; i++) {
    const { data: exists } = await db.from("events").select("id").eq("share_code", shareCode).maybeSingle()
    if (!exists) break
