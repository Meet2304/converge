...
export default async function MatchPage({
  ...
  const { data: match } = await db
    .from("matches")
    .select(
      "*, a:participations!matches_participation_a_id_fkey(id, nickname, avatar_url), b:participations!matches_participation_b_id_fkey(id, nickname, avatar_url)",
    )
    .eq("id", matchId)
  ...
  return (
    <main className="mx-auto flex w-full max-w-lg flex-1 flex-col items-center justify-center gap-6 px-6 py-16 text-center">
      ...
      <div className="flex items-center gap-4">
        ...
        <img
          src={(match.a as { avatar_url: string }).avatar_url}
          alt=""
          className="size-16 rounded-full"
        />
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={(match.b as { avatar_url: string }).avatar_url}
          alt=""
          className="size-16 rounded-full"
        />
      </div>
      ...
      <Card className="w-full border-white/10">
        ...
        <CardContent className="flex flex-col gap-2">
          {match.conversation_id ? (
            <Button
              nativeButton={false}
              render={<Link href={`/event/${event.share_code}/chat/${match.conversation_id}`} />}
            >
              Open chat
            </Button>
          ) : null}
