...
import { consentAndEnableSharing, dropMeetupPin, upsertSparseLocation } from "@/app/actions/map";
...
export function MapCanvas(props: Props) {
  ...
  const [findTargetId, setFindTargetId] = useState<string | null>(
    props.findTarget?.participationId ?? null,
  );
  ...
  const channelRef = useRef<ReturnType<ReturnType<typeof createBrowserSupabase>["channel"]> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    setPrevDist(d);
  }, [me, findTarget, sharing, prevDist]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className="flex flex-col gap-4">
      ...
      {sharing ? (
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <form className="flex flex-col gap-2" action={dropMeetupPin}>
...
function StubPlane({
  ...
  return (
    <div className="relative h-[420px] w-full bg-[radial-gradient(circle_at_center,#141414,transparent_60%)]">
      ...
      {me ? <Dot {...project(me.lat, me.lng)} color="#f2f2f2" label="You" /> : null}
      {peers.map((p) => (
        <Dot
